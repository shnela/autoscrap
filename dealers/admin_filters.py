...
from dealers.models import DealerCar
...
class CarPrefixFilter(SimpleListFilter):
  ...
  def lookups(self, request, model_admin):
    car_info_prefixes = set(
      (d.info.split()[0], d.info.split()[0]) for d in DealerCar.objects.all()
    )
  ...
  def queryset(self, request, queryset):
    ...
    if car_info_prefix is not None:
      matched_cars = DealerCar.objects.filter(
        info__istartswith=car_info_prefix
