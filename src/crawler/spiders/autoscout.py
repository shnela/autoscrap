...
class AutoscoutDealerStatsSpider(scrapy.Spider):
    ...
    def __init__(self, dealers, km_to=2500, register_from=2018):
        ...
        self.dealer_url_args = "?atype=C&kmto={}&fregfrom={}".format(km_to, register_from)
    ...
    def parse(self, response):
        # parse dealer
        cars_count_str = response.xpath('//p[contains(@class, "dp-list__title__count")]/text()').extract_first()
...
class AutoscoutDealerCarsSpider(scrapy.Spider):
    ...
    def start_requests(self, page=1):
        url = "{}/{}&page={}".format(self.dealer.vehicles_url, self.dealer_url_args, page)
        yield scrapy.Request(
            url=url,
            meta={"page": 1},
    ...
    def parse_cars_list(self, response):
        car_frames = response.xpath('//div[contains(@class, "dp-listing-item")]')
        for car_frame in car_frames:
            url = car_frame.xpath('.//a[contains(@class, "dp-link")]' "/@href").extract_first()
